...
class AppWeatherCurrentSummaryFragment extends StatelessWidget {
  final AppWeatherSingleSummaryResponseDto? weather;

  const AppWeatherCurrentSummaryFragment({super.key, this.weather});
  ...
  Widget _displayNorthSymbol(BuildContext context, BoxConstraints constraints) {
    ...
    return AppSquarePositionedTextComponent(
        squareSize: baseSize,
        text: AppL18nConfig.get(context).weather_wind_direction_north,
        left: baseSize / 2,
        top: baseSize / 30,
  ...
  Widget _displaySouthSymbol(BuildContext context, BoxConstraints constraints) {
    ...
    return AppSquarePositionedTextComponent(
        squareSize: baseSize,
        text: AppL18nConfig.get(context).weather_wind_direction_south,
        left: baseSize / 2,
        bottom: baseSize / 30,
        color: Theme.of(context).colorScheme.onBackground.withOpacity(0.7),
        textDensity: 3.5,
  ...
  Widget _displayEastSymbol(BuildContext context, BoxConstraints constraints) {
    ...
    return AppSquarePositionedTextComponent(
      squareSize: baseSize,
      text: AppL18nConfig.get(context).weather_wind_direction_east,
      right: baseSize / 30,
      top: baseSize / 2,
      color: Theme.of(context).colorScheme.onBackground.withOpacity(0.7),
      textDensity: 3.5,
  ...
  Widget _displayWestSymbol(BuildContext context, BoxConstraints constraints) {
    ...
    return AppSquarePositionedTextComponent(
      squareSize: baseSize,
      text: AppL18nConfig.get(context).weather_wind_direction_west,
      left: baseSize / 30,
      top: baseSize / 2,
      color: Theme.of(context).colorScheme.onBackground.withOpacity(0.7),
      textDensity: 3.5,
  ...
  Widget _displayRainText(BuildContext context, BoxConstraints constraints) {
    ...
    if (weather?.lastRain != null) {
      ...
      if (duration != null) {
        rainText = AppL18nConfig.get(context).weather_last_rain_duration(duration);
      }
