...
import 'package:weatherapp_ui/fragments/summary/detail/app_weather_detail_summary_fragment.dart';
import 'package:weatherapp_ui/fragments/summary/list/app_list_summary_fragment.dart';
import 'package:weatherapp_ui/fragments/summary/list/item/app_weather_list_summary_item_fragment.dart';
...
class AppWeatherListSummaryFragment extends StatelessWidget {
  final AppStationResponseDto? station;
  final AppCalendarEnum? type;

  const AppWeatherListSummaryFragment({super.key, this.station, this.type});
    ...
    return Consumer<AppWeatherDayAggregationSummaryProvider>(
      builder: (context, provider, widget) {
        return AppListSummaryFragment(
          itemBuilder: (d) => AppWeatherListSummaryItemFragment(
              data: d,
              time: d.day,
    ...
    Navigator.push(
        ...
        PageRouteBuilder(
          pageBuilder: (context, animation1, animation2) => AppDetailDataPage(
              title: title,
              child: AppWeatherDetailSummaryFragment(
                time: time,
                type: type,
              )),
        ));
