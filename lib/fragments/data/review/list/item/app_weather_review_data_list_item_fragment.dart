...
class AppWeatherReviewDataListItemFragment
    extends AppReviewDataListItemFragment<AppWeatherSummaryDataResponseDto> {
  ...
  Color tapColor(BuildContext context) {
    return AppColorService()
        .temperatureToColor(context, super.data?.temperatureAvg);
  }
  ...
  Widget _windDataItem(BuildContext context) {
    AppWeatherSummaryDataResponseDto? weather = super.data;
    Color color = Color.alphaBlend(const Color.fromRGBO(111, 111, 111, 0.7),
        Theme.of(context).colorScheme.onBackground);
    String text = weather?.windMax != null && weather?.windMax != 0
        ? "${weather?.windMax} km/h"
        : "";
    IconData? icon = weather?.windMax != null && weather?.windMax != 0
        ? AppIcons.wind
        : null;
  ...
  Widget _rainTotalDataItem(BuildContext context) {
    AppWeatherSummaryDataResponseDto? weather = super.data;
    Color color = Color.alphaBlend(const Color.fromRGBO(0, 175, 255, 0.7),
        Theme.of(context).colorScheme.onBackground);
    String text = weather?.rainTotal != null && weather?.rainTotal != 0
        ? "${weather?.rainTotal} l/m²"
        : "";
    IconData? icon = weather?.rainTotal != null && weather?.rainTotal != 0
        ? AppIcons.rain
        : null;
    return _dataItem(context,
        data: text,
  ...
  Widget _dataItem(BuildContext context,
      {String? data, double? width, IconData? icon, TextStyle? styleBase}) {
    return SizedBox(
      width: width,
      child: Row(
        children: [
