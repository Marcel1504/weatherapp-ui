...
abstract class AppDetailDataApiService<LIST extends AppListResponseDto> {
  Future<LIST?> getDayDetails(String? stationCode, String? day) async {
    Response res = await get(AppApiService().restUrl("${getDayDetailsBaseEndpoint()}"
        "?station=$stationCode"
        "&day=$day"
        "&sort=${getDefaultSort()}"));
  ...
  Future<LIST?> getMonthsDetails(String? stationCode, String? yearMonth) async {
    Response res = await get(AppApiService().restUrl("${getMonthDetailsBaseEndpoint()}"
        "?station=$stationCode"
  ...
  Future<LIST?> getYearDetails(String? stationCode, String? year) async {
    Response res = await get(AppApiService().restUrl("${getYearDetailsBaseEndpoint()}"
        "?station=$stationCode"
        "&year=$year"
        "&size=12"
        "&page=0"
